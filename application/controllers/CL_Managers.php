<?php
/**
 * This controller serves all the actions performed by claim managers
 * @since         0.2.0
 */
 ...
 * ...
 */
class CL_Managers extends CI_Controller {
	...
	{
        $this->auth->checkIfOperationIsAllowed('clmanagers_stats');
        ...
        $this->load->view('clmanagers/stats', $twig);
    }
    
    /**
	 * PAGE - Main task of Manager - list of payments are waiting for Manager to approve
	 */
    public function manager_approval($leader = '', $member = '')
	{
        $this->auth->checkIfOperationIsAllowed('clmanagers_manager_approval');
        $twig = getUserContext($this);
        ...
        $twig['leader'] = $leader;
        $this->load->view('clmanagers/clmanagers', $twig);
    }
	...
	{
        $this->auth->checkIfOperationIsAllowed('clmanagers_new');
        $twig = getUserContext($this);
        $this->lang->load('columns');
        $this->load->model('users_model');
        $twig['page'] = 'new';
	...
	{
        $this->auth->checkIfOperationIsAllowed('clmanagers_leader_approval');
        $twig = getUserContext($this);
        $this->lang->load('columns');
        $this->load->model('users_model');
	...
	{
        $this->auth->checkIfOperationIsAllowed('clmanagers_leader_rejected');
        $twig = getUserContext($this);
        $this->lang->load('columns');
        $this->load->model('users_model');
	...
	{
        $this->auth->checkIfOperationIsAllowed('clmanagers_manager_rejected');
        $twig = getUserContext($this);
        $this->lang->load('columns');
        $this->load->model('users_model');
	...
	{
        $this->auth->checkIfOperationIsAllowed('clmanagers_director_rejected');
        $twig = getUserContext($this);
        $this->lang->load('columns');
        $this->load->model('users_model');
    ...
    /**
	 * PAGE - Show all Payments which are waiting for Director to approve
	 */
    public function director_approval($leader = '', $member = '')
	{
        $this->auth->checkIfOperationIsAllowed('clmanagers_director_approval');
        $twig = getUserContext($this);
        $this->lang->load('columns');
        $this->load->model('users_model');
    ...
    public function request_director($paym_id)
    {
        ...
        if ($payment['IS_DELETED'] OR ! in_array($payment['TF_STATUS_ID'], [TF_STATUS_NEW, TF_STATUS_LEADER_APPROVAL, TF_STATUS_LEADER_REJECTED, TF_STATUS_MANAGER_APPROVAL, TF_STATUS_MANAGER_REJECTED]))
        {
            redirect('forbidden');
        }
        $this->auth->checkIfOperationIsAllowed('request_director');
        
        $this->db->trans_start();
        $this->payments_model->set_value($paym_id, 'TF_STATUS_ID', TF_STATUS_DIRECTOR_APPROVAL);
        $this->payments_history_model->add($paym_id, HIST_TYPE_REQUEST_DIRECTOR, HIST_FIELD_TF_STATUS, $payment['TF_STATUS_ID'], TF_STATUS_DIRECTOR_APPROVAL);
    ...
    /**
     * ACTION - reject leader's request
     * @param int $paym_id
     */
    public function manager_reject($paym_id)
    {
        $payment = $this->payments_model->get_payment($paym_id);
        if (empty($payment))
        {
        ...
        $this->auth->checkIfOperationIsAllowed('manager_reject');
        ...
        $this->payments_history_model->add($paym_id, HIST_TYPE_MANAGER_REJECT, HIST_FIELD_TF_STATUS, TF_STATUS_MANAGER_APPROVAL, TF_STATUS_MANAGER_REJECTED);
        $this->notes_model->add($paym_id, NOTE_TYPE_MANAGER_REJECT, $this->input->post('reject_reason'));
        $this->db->trans_complete();
        redirect("payments/$paym_id");
    }
    
    /**
     * ACTION - Manager takes over a request in Finance
     * @param int $paym_id
     */
    public function take_over($paym_id)
    {
        ...
        $this->payments_history_model->add($paym_id, HIST_TYPE_MANAGER_TAKE_OVER, HIST_FIELD_TF_STATUS, TF_STATUS_APPROVED, TF_STATUS_MANAGER_APPROVAL);
        $this->notes_model->add($paym_id, NOTE_TYPE_MANAGER_TAKE_OVER, $this->input->post('reject_reason'));
        $this->db->trans_complete();
        redirect("payments/$paym_id");
    }
}
